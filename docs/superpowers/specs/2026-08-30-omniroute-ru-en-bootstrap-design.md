...
- OmniRoute model route: `kmc/k3-256k` (current test route)
...
2. The Russian-tuned Faster Whisper Small INT8 transcribes Russian or English
   speech locally with CPU inference and per-turn language detection.
...
speech profile remains below the initial 5.9 GB Parakeet/Qwen profile. GPU
enablement is not hidden inside the bootstrap.

Live transcription is disabled for this CPU profile. The generic default runs
Faster Whisper on partial audio and then repeats inference on the completed
utterance; measured on this host, that duplicate work added about 3.8 seconds
to a short turn.

The VAD silence threshold is 500 ms rather than the generic 64 ms. The shorter
threshold split a single synthetic sentence at a natural pause and forced a
second full Whisper pass; the longer threshold trades at most 436 ms of final
silence for avoiding several seconds of duplicate CPU inference.

## Language policy

The Russian-tuned Faster Whisper Small INT8 checkpoint runs with automatic
language detection and supports both `ru` and `en`. `enable_lang_prompt` is
enabled so a confidently detected language is propagated to the LLM. The
session prompt applies these rules:
...
## Current OmniRoute route

The gateway health endpoint is healthy. The generic `auto/chat` route spent
6-12 seconds selecting among providers and was therefore replaced with the
explicit `kmc/k3-256k` test route. A direct Russian smoke request was coherent.
This design guarantees a locally hosted gateway, not fully offline LLM
inference.
...
change may switch `model_name` from the current test route to a verified offline
route once that service passes its own health and Russian-language smoke tests.
...
The active speech-model caches are about 254 MB for the Russian Faster Whisper
Small INT8 and 386 MB for Supertonic 3. Any failure must retain the exact
command and error; the launcher must not silently switch to a remote STT or TTS
provider.
...
- Configuration parsing selects the Russian Faster Whisper Small INT8 on CPU,
  `kmc/k3-256k` through OmniRoute Chat Completions, and Supertonic 3 with
  automatic RU/EN handling.
...
- GPU and model-download blockers are reported accurately rather than being
  presented as a successful end-to-end voice run.
